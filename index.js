...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Tchau Corno(a) @${num.split('@')[0]} ja foi tarde 👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ Por favor, aguarde um pouco, caso não obtenha, tente novamente!⌛',
				success: '✔️ Com Sucesso ✔️',
				error: {
					stick: '❌ Falha, ocorreu um erro ao converter a imagem/gif em figurinha ❌',
					Iv: '❌ Link tidak valid ❌'
				},
				only: {
					group: '❌ Este comando só pode ser usado em grupos! ❌',
					ownerG: '❌ Este comando só pode ser usado pelo grupo proprietário! ❌',
					ownerB: '❌ Este comando só pode ser ultilizado pelo Herberth! ❌',
			...
			switch(command) {
				...
				case 'sticker':
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`Envie foto/gif com a legenda  ${prefix}sticker ou marque alguma imagem ou gif que ja foi enviada!`)
					}
					break
				case 'gtts':
					...
					rano = getRandom('.ogg')
					dtt.length > 600
					? reply('A maior parte do texto é asneira')
				...
				case '+18':
					...
					buffer = await getBuffer(`https://i.imgur.com/zdJT6rD.jpg`)
				...
				case 'hentai':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Aqui está sua imagem!'})
					break
				case 'bomdia':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/7VL9cFf.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Bom dia, o sol ja nasceu na fazendinha!'})
					break
				case 'boatarde':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/xpDsMEj.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Boa tarde, clan😎'})
					break
				case 'boanoite':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/WPruBwj.jpg`)
                ...
                case 'membros2':
                 ...
                 case 'membros3':
					...
					break
				case 'limpar':
					if (!isOwner) return reply('❌ Este Comando só funciona, se você for o dono do bot 🖕❌')
